...
router.put("/:gunId", async (req, res) => {
  // Update specific gun
  try {
    ...
    const updatedGun = await Gun.findByIdAndUpdate(req.params.gunId, req.body, {
      new: true,
    });
...
router.get("/:gunId", async (req, res) => {
  // Get specific gun
  try {
    const gun = await Gun.findById(req.params.gunId);
    if (!gun) {
...
router.delete("/:gunId", async (req, res) => {
  // Delete specific gun
  try {
